...
begin
  tc := GetTickCount;
  // дл€ отправки только первого сообщени€ из очереди надо провер€ть интервал между сообщени€ми, так как
  // при создании файла генерируютс€ два-три сообщени€ подр€д: первое - FILE_ACTION_ADDED, второе и третье - FILE_ACTION_MODIFIED, отправл€ем только сообщение о создании
  // также при изменении файла следуют подр€д два сообщени€ FILE_ACTION_MODIFIED - отправл€ем только одно
    ...
    begin
      if not ReadDirectoryChangesW(
        ...
        begin
          ...
          Terminate;
          Break; // невозможно поставить запрос наблюдени€ каталога в очередь
        end;
      WaitResult := WaitForMultipleObjects(2, @FEventsToWait, False, INFINITE);
      if WaitResult = WAIT_OBJECT_0 then
        begin
          Terminate;
          Break; // получен внешний запрос на окончание наблюдени€
        end
      else if WaitResult <> WAIT_OBJECT_0 + 1 then
        begin
          DoOnError(SysErrorMessage(GetLastError));
          Terminate;
          Break; // неизвестна€ ошибка
        end
      else
        begin
          // закончилась операци€ чтени€ изменений в каталоге
          if not GetOverlappedResult (FDirectoryHandle, FOverlapped, BytesRead, False) then
            begin
              DoOnError(SysErrorMessage(GetLastError));
              Terminate;
              Break;  // неизвестна€ ошибка при попытке получени€ результата окончани€
                      // асинхронной операции ввода-вывода
            end;
          // сейчас в буфере находитс€ BytesRead байт информации об изменени€х в каталоге
          try
